...
from kanon_shogi.network.policy_value_resnet import *
from kanon_shogi.player.base_player import *
from kanon_shogi.uct.uct_node import *
...
class MCTSCpuPlayer(BasePlayer):
    def __init__(self):
        super().__init__()
        #モデルファイル
    ...
    def go(self):
        ...
        if self.board.move_number < 10:
            #訪問回数に応じた確率で指し手を選択する
            selected_index = np.random.choice(np.arange(child_num),
                                                p=child_move_count/sum(child_move_count))
        ...
        for i in range(child_num):
            print("{:3}:{:5} move_count:{:4} nn_rate:{:.5f} win_rate:{:.5f}"
                .format(i,
                        child_move[i].usi(),
                        child_move_count[i],
        ...
        #評価値の通知
        print("info nps {} time {} nodes {} hashfull {} score cp {} pv {}"
            .format(int(current_node.move_count / finish_time),
    ...
    def expand_node(self, board):
        ...
        index = self.node_hash.find_same_hash_index(board.zobrist_hash(),
                    board.turn, board.move_number)
        ...
        #空のインデックスを探す
        index = self.node_hash.search_empty_index(board.zobrist_hash(),
                    board.turn, board.move_number)
